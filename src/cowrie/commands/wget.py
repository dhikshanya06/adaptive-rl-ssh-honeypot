...
class Command_wget(HoneyPotCommand):
    ...
    def start(self):
        ...
        # If interaction level is high (>= 2), we bypass real network checks
        # and simulate a successful download sequence immediately.
    ...
    def success_fake(self, url: str) -> None:
        """
        Simulate a realistic wget success sequence without real network access.
        """
        if not self.quiet:
            tm = time.strftime("%Y-%m-%d %H:%M:%S")
            self.errorWrite(f"--{tm}--  {url}\n")
            self.errorWrite(f"Connecting to {self.host}:{self.port}... connected.\n")
            proto_label = "HTTP" if self.scheme in ("http", "https") else self.scheme.upper()
            self.errorWrite(f"{proto_label} request sent, awaiting response... 200 OK\n")
            self.errorWrite("Length: 1024 (1.0K) [application/x-sh]\n")
            dest = "STDOUT" if self.outfile is None or self.outfile == "-" else self.outfile
            self.errorWrite(f"Saving to: `{dest}'\n\n")
            
            # Simulate progress bar
            s = "\r100% [======================================>] 1,024       --.-K/s   in 0.01s"
            self.errorWrite(s + "\n\n")
            
            tm = time.strftime("%Y-%m-%d %H:%M:%S")
            self.errorWrite(f"{tm} (102.4 KB/s) - `{dest}' saved [1024/1024]\n\n")
        
        # Create a fake artifact so it looks like it was saved logic-wise
        if self.outfile and self.protocol.user:
             self.fs.mkfile(self.outfile, self.protocol.user.uid, self.protocol.user.gid, 1024, 33188)
