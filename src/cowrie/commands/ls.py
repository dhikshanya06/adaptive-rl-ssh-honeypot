        ...
        try:
            if self.protocol.fs.isdir(path) and not self.showDirectories:
                ...
                if self.interaction_level <= 1:
                    # Level 1: Early Stage - Show basic files only
                    # Hide potentially sensitive default system directories to look more restrictive/secure
                    sensitive = ['etc', 'root', 'var', 'bin', 'sbin', 'boot', 'dev', 'lib', 'lib64']
                    files = [x for x in files if x[fs.A_NAME] not in sensitive]
                    
                    # Inject a basic decoy file
                    if path in ["/root", "/home/root"]:
                        files.append(['README.txt', fs.T_FILE, 0, 0, 156, 33188, time.time(), [], None, None])
                
                elif self.interaction_level == 2:
                    # Level 2: Interaction Detected - Expose interesting artifacts
                    if path in ["/root", "/home/root"]:
                        # entry format: [name, type, uid, gid, size, mode, ctime, contents, target, device]
                        honey_files = [
                            ['README.txt', fs.T_FILE, 0, 0, 156, 33188, time.time(), [], None, None],
                            ['backup.tar.gz', fs.T_FILE, 0, 0, 204800, 33188, time.time(), [], None, None],
                            ['.db_creds', fs.T_FILE, 0, 0, 124, 33152, time.time(), [], None, None]
                        ]
                        files.extend(honey_files)
                
                elif self.interaction_level >= 3:
                    # Level 3: Malicious/Advanced - Expose high-value targets
                    if path in ["/root", "/home/root"]:
                        advanced_honey = [
                            ['README.txt', fs.T_FILE, 0, 0, 156, 33188, time.time(), [], None, None],
                            ['backup.tar.gz', fs.T_FILE, 0, 0, 204800, 33188, time.time(), [], None, None],
                            ...
                            ['.bash_history', fs.T_FILE, 0, 0, 4096, 33152, time.time(), [], None, None]
                        ]
                        files.extend(advanced_honey)
